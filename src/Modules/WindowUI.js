 ...
 */
function WindowUI(name, question, answers, correct_answer, scene, ultima_pregunta) {
    ...
    // contenido
    var windowMesh = BABYLON.MeshBuilder.CreatePlane("windowUI"+ name, {
        width: width * (0.001),
        height: height * (0.001),
        sideOrientation: BABYLON.Mesh.DOUBLESIDE
    }, scene);
...
export function WindowsUI(preguntas_respuestas_sol, subScene) {

    var ventanas = [];

    for(var i = 0; i <= preguntas_respuestas_sol["test"].length - 1 ; i++) {

        var ultima_pregunta = i === preguntas_respuestas_sol["test"].length - 1 ? true : false;

        var ventana = WindowUI("Pregunta "+ (i + 1), preguntas_respuestas_sol["test"][i]["question"], preguntas_respuestas_sol["test"][i]["answers"], preguntas_respuestas_sol["test"][i]["correctAnswer"], subScene, ultima_pregunta);
